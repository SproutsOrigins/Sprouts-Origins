...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/Sprouts-OriginsParams"
    else
        PARAMS_DIR="$HOME/.Sprouts-Origins-params"
    fi
fi
...
Failed to fetch the Sprouts-Origins zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Sprouts-Origins - fetch-params.sh

This script will fetch the Sprouts-Origins zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Sprouts-Origins zkSNARK parameters. Note that it is
