...
#define URI_SCHEME "Sprouts-Origins"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter Sprouts-Origins address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Sprouts-Origins: URI
...
{
    // Convert Sprouts-Origins:// to Sprouts-Origins:
    //
    //    Cannot handle this later, because Sprouts-Origins:// will cause Qt to see the part after // as host,
...
{
    return GetAutostartDir() / "Sprouts-Origins.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Sprouts-Origins.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Sprouts-Origins app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Sprouts-Origins app to startup item list
