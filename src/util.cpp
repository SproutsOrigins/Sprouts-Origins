...
const char * const PIVX_CONF_FILENAME = "Sprouts-Origins.conf";
const char * const PIVX_PID_FILENAME = "Sprouts-Origins.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Sprouts-Origins only features
// Masternode
...
#else
    const char* pszModule = "Sprouts-Origins";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Sprouts-Origins
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Sprouts-Origins
// Mac: ~/Library/Application Support/Sprouts-Origins
// Unix: ~/.Sprouts-Origins
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sprouts-Origins";
#else
    fs::path pathRet;
    ...
    return pathRet / "Sprouts-Origins";
#else
    // Unix
    return pathRet / ".Sprouts-Origins";
#endif
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Sprouts-OriginsParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Sprouts-OriginsParams
    // Mac: ~/Library/Application Support/Sprouts-OriginsParams
    // Unix: ~/.Sprouts-Origins-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sprouts-OriginsParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "Sprouts-OriginsParams";
#else
    // Unix
    return pathRet / ".Sprouts-Origins-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty Sprouts-Origins.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override Sprouts-Origins.conf
