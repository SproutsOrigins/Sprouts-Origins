...
- [Development guidelines](#development-guidelines)
    - [General Sprouts-Origins Core](#general-Sprouts-Origins-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Sprouts-Origins Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Sprouts-Origins Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/sappcoin-com/Sprouts/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_Sprouts-Origins
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_Sprouts-Origins --log_level=test_suite
$ valgrind -v --leak-check=full src/Sprouts-Originsd -printtoconsole
```
...
However, in open source software such as Sprouts-Origins Core, where everyone uses
...
pay attention to for reviewers of Sprouts-Origins Core code.

General Sprouts-Origins Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Sprouts-Origins Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Sprouts-Origins Core so that they can be integrated
...
file descriptors it will cause problems with Sprouts-Origins's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:sappcoin-com/Sprouts.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `Sprouts-Origins-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `Sprouts-Origins-cli` and the GUI debug console use this table to determine how to
